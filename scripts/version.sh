...
#   scripts/version.sh --write-python   # regenerate the Python package's _version.py
set -euo pipefail
...
case "${mode}" in
    ...
    --tag)
        ...
        ;;
    --write-python)
        # hatchling reads the version from this file, so anything that builds or
        # installs the Python package needs it to exist — tests included. It is
        # generated rather than committed so it can never disagree with the tag.
        target="${REPO_ROOT}/python/sensorctl/src/sensorctl/_version.py"
        cat > "${target}" <<EOF
# Generated by scripts/version.sh --write-python — do not edit, do not commit.
__version__ = "${version}"
EOF
        printf '%s\n' "${version}"
        ;;
    *)
        printf 'usage: %s [--docker|--tag|--write-python]\n' "$0" >&2
        exit 2
        ;;
esac
